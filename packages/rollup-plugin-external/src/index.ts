...
export default function lunaPressExternal(options: LunaPressExternalOptions): Plugin {
    const { globals, include, exclude, dynamicWrapper = defaultDynamicWrapper } = options
            ...
            walk(ast, {
                enter(this, node: Node, parent: Node | null) {
                    ...
                    if (dynamicGlobal && isNodeWithStartAndEnd(node)) {
                        magicString.overwrite(node.start, node.end, dynamicWrapper(dynamicGlobal))
                        isTouched = true
                        this.skip()
                        return
                    }
                    ...
                    if (
                        ...
                        if (bindings.has(name) && !scope.contains(name)) {
                            ...
                            if (global !== undefined && isNodeWithStartAndEnd(node)) {
                                magicString.overwrite(node.start, node.end, global, {
                                    contentOnly: true,
                                })
                                isTouched = true
                            }
